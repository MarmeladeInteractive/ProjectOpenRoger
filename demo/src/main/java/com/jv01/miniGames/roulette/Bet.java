...
public class Bet {
    ...
    public long getResult(String number, String colors){
        ...
        if(numberInt!=0)if((simpleNumberBet[numberInt-1]>0)){
            result += simpleNumberBet[numberInt-1]*36;
        }
        ...
        if(numberInt!=0)switch (numberInt % 3) {
            case 0:
                if (yIndexMultipleNumber * 2 + 1 < 21) {
                    result += (squareNumberBet[yIndexMultipleNumber * 2 + 1] > 0) ? squareNumberBet[yIndexMultipleNumber * 2 + 1] * 8 : 0;
                }
                if (yIndexMultipleNumber - 1 >= 0) {
                    result += (squareNumberBet[(yIndexMultipleNumber - 1) * 2 + 1] > 0) ? squareNumberBet[(yIndexMultipleNumber - 1) * 2 + 1] * 8 : 0;
                }
                break;
            case 1:
                if (yIndexMultipleNumber * 2 < 21) {
                    result += (squareNumberBet[yIndexMultipleNumber * 2] > 0) ? squareNumberBet[yIndexMultipleNumber * 2] * 8 : 0;
                }
                if (yIndexMultipleNumber - 1 >= 0) {
                    result += (squareNumberBet[(yIndexMultipleNumber - 1) * 2] > 0) ? squareNumberBet[(yIndexMultipleNumber - 1) * 2] * 8 : 0;
                }
                break;
            case 2:
                if (yIndexMultipleNumber * 2 < 21) {
        ...
        if(numberInt!=0)if(numberInt-1<=32)if(double01NumberBet[numberInt-1] > 0)result += double01NumberBet[numberInt-1] * 17;
        if(numberInt!=0)if((numberInt-1-3)>=0)if(double01NumberBet[numberInt-1-3] > 0)result += double01NumberBet[numberInt-1-3] * 17;
        
        if(numberInt!=0)switch (numberInt % 3) {
            case 0:

                    result += (double02NumberBet[yIndexMultipleNumber*2+1] > 0) ? double02NumberBet[yIndexMultipleNumber *2+1] * 17 : 0;

                    result += (twoToOneNumberBet[2] > 0) ? twoToOneNumberBet[2] * 2 : 0;
                break;
            case 1:

                    result += (double02NumberBet[yIndexMultipleNumber*2] > 0) ? double02NumberBet[yIndexMultipleNumber *2] * 17 : 0;

                    result += (twoToOneNumberBet[0] > 0) ? twoToOneNumberBet[0] * 2 : 0;
                break;
            case 2:
                    result += (double02NumberBet[(yIndexMultipleNumber*2+1)] > 0) ? double02NumberBet[(yIndexMultipleNumber *2+1)] * 17 : 0;
                    result += (double02NumberBet[(yIndexMultipleNumber*2)] > 0) ? double02NumberBet[(yIndexMultipleNumber*2)] * 17 : 0;

                    result += (twoToOneNumberBet[1] > 0) ? twoToOneNumberBet[1] * 2 : 0;
                break;
            default:
                break;
        }

        if(numberInt!=0)if(thirdNumberBet[(int)Math.ceil(numberInt/12)]>0)result+=thirdNumberBet[(int)Math.ceil(numberInt/12)]*2;

        if(colors.equals("noir"))if(otherNumberBet[3]>0)result+=otherNumberBet[3];
        if(colors.equals("rouge"))if(otherNumberBet[2]>0)result+=otherNumberBet[2];
        if(numberInt!=0)if(numberInt<=18){
            if(otherNumberBet[0]>0)result+=otherNumberBet[0];
        }else{
            if(otherNumberBet[5]>0)result+=otherNumberBet[5];
        }
        if(numberInt!=0)if(numberInt%2==0){
            if(otherNumberBet[1]>0)result+=otherNumberBet[1];
        }else{
            if(otherNumberBet[4]>0)result+=otherNumberBet[4];
        }


        
        if(totalBet<=result){
            roulette.scoreLabel.setText(String.valueOf( "Gains : " + result ));
            if(roulette.isInGame)giveMoneyToPlayer(result);
        }else{
            roulette.scoreLabel.setText(String.valueOf( "Losses : " + totalBet ));
            if(roulette.isInGame)takeMoneyToPlayer(totalBet);
        }
